...
public class Engine {
	...
	float getPowerAtRpm(float[] powerArray, float rpm){
		...
		selector = Mathf.Clamp (selector, 0, powerArray.Length - 2);
...
public class Sound {
	...
	public void Main(CarController controller){
		...
		if(controller.engine.sound != null){
			#region pitch
			if (originalPitch < 0)
				originalPitch = controller.engine.sound.pitch;
			...
			controller.engine.sound.volume = targetVolume * volumeScale;
			controller.engine.sound.mute = false;
			#endregion
		}
...
public class Inputs {
	...
	public bool bypass;
	[HideInInspector]
	public float bypassX;
	[HideInInspector]
	public float bypassY;
...
public class CarController : MonoBehaviour {
	...
	public void SetPath(GameObject pathGroup)
	{
		//Debug.Log("SET PATH");
		this.pathGroup = pathGroup.transform;
		FollowPath = true;
		GetPath();
	}
	...
	void GetPath()
	{
		...
		var path_objs = pathGroup.GetComponentsInChildren<Transform>();
